                ...
                .frame(height: size.height * 0.15)
                    ...
                    ZStack {
                        Image("heartpic")
                            ...
                            .shadow(color: accentRed.opacity(0.5), radius: glowRadius, x: 0, y: 0).shadow(color: accentRed.opacity(0.3), radius: glowRadius * 1.5, x: 0, y: 0)
                .scaleEffect(heartScale)
                            .position(x: size.width / 2, y: labelAreaHeight / 2)
                        
                        ForEach(Array(features.enumerated()), id: \.offset) { index, feature in
                FeatureLabelView(feature: feature, accentColor: accentRed)
                                .scaleEffect(visibleLabels.contains(index) ? 1.0 : 0.3)
                                .opacity(visibleLabels.contains(index) ? 1.0 : 0)
                                .position(x: size.width * feature.position.x, y: labelAreaHeight * feature.position.y)}
                    }
